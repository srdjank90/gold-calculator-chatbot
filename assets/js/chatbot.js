jQuery(document).ready(function ($) {
  ...
  function initChatbot() {
    ...
    const greetingMessage =
      (typeof gold_suggestions_ajax !== "undefined" && gold_suggestions_ajax.persona_greeting) ||
  ...
  function loadAllQuestionsFromDatabase() {
    $.ajax({
      ...
      data: {
        ...
        nonce: (typeof gold_suggestions_ajax !== "undefined" && gold_suggestions_ajax.nonce) || (typeof gcc_ajax !== "undefined" && gcc_ajax.nonce) || "",
      },
      dataType: "json",
        ...
        if (response.success) {
          ...
          setTimeout(() => {
            if (questions.length > 0) {
              ...
              addBotMessage("Izvinite, nema dostupnih pitanja. Molimo kontaktirajte nas direktno.");
            }
          }, 1500);
        } else {
  ...
  function showNextValidQuestion() {
    ...
    for (let i = chatbotState.currentQuestion; i < questions.length; i++) {
      ...
      if (attributes.budget && chatbotState.userAnswers.budget) isAnswered = true;
      if (attributes.product_type && chatbotState.userAnswers.product_type) isAnswered = true;
      if (attributes.combo_percentage && chatbotState.userAnswers.combo_percentage) isAnswered = true;
      if (attributes.delivery_method && chatbotState.userAnswers.delivery_method) isAnswered = true;
  ...
  function calculateTotal() {
    chatbotState.totalValue = chatbotState.selectedProducts.reduce((sum, product) => {
      if (product.total_price) {
        return sum + parseFloat(product.total_price_eur || product.total_price);
      }
      return sum + parseFloat(product.final_price_eur || product.final_price) * (product.quantity || 1);
    }, 0);
  }
  ...
  function showProductSelection() {
    ...
    if (chatbotState.selectedProducts.length === 0) {
      addBotMessage("Nema dostupnih proizvoda za vaš budžet. Molimo kontaktirajte nas za više informacija.");
      showContactForm();
      return;
    }
  ...
  function showContactForm() {
    const actionText = chatbotState.quoteType === "email" ? "email sa ponudom" : "poziv od trgovca";
    addBotMessage(`Odlično! Da biste dobili ${actionText}, molimo unesite vaše podatke:`);
  ...
  function handleContactSubmission(e) {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          addBotMessage("Hvala! Vaš zahtev je uspešno poslat. Uskoro ćemo vam se javiti sa detaljnom ponudom.");
          $("#gcc-products-container").html('<div class="gcc-success-message">Uspešno poslato! ✅</div>');

          setTimeout(() => {
            const resetHtml = '<button class="gcc-cta-btn gcc-primary" onclick="resetChatbot()">Napravi novi proračun</button>';
            $("#gcc-products-container").html(resetHtml);
          }, 2000);
        } else {
          alert("Greška: " + (response.data ? response.data.message : "Nepoznata greška"));
          resetSubmitButton();
        }
      },
      error: function () {
  ...
  function updateTotal() {
    ...
    $("#gcc-total-rsd").text((chatbotState.totalValue * chatbotState.exchangeRate).toFixed(0) + " RSD");
  }
  ...
  function handleScheduleMeeting() {
    ...
    $.ajax({
      ...
      type: "POST",
      data: {
        action: "gcc_get_calendly_url",
        nonce: (typeof gcc_ajax !== "undefined" && gcc_ajax.nonce) || "",
      },
      dataType: "json",
      success: function (response) {
            ...
            const calendlyIframe = `
              <div class="gcc-calendly-container">
                <div class="gcc-calendly-header">
                  ...
                  <button class="gcc-close-calendly" onclick="closeCalendlyAndShowReset()">✕</button>
                </div>
                <iframe src="${calendlyUrl}" width="100%" height="1200" frameborder="0" scrolling="yes" style="min-height: 650px;"></iframe>
              </div>
            `;
  ...
  function getPersonaAvatarHtml() {
    const personaImage = (typeof gold_suggestions_ajax !== "undefined" && gold_suggestions_ajax.persona_image) || (typeof gcc_ajax !== "undefined" && gcc_ajax.persona_image) || "";
  ...
  function getUserAvatarHtml() {
    const userAvatarImage = (typeof gold_suggestions_ajax !== "undefined" && gold_suggestions_ajax.user_avatar_image) || (typeof gcc_ajax !== "undefined" && gcc_ajax.user_avatar_image) || "";
  ...
  // Close Calendly and show reset button
  function closeCalendlyAndShowReset() {
    // Hide the calendar window
    $(".gcc-calendly-container").hide();

    // Show the reset button section
    const resetHtml = '<button class="gcc-cta-btn gcc-primary" onclick="resetChatbot()">Napravi novi proračun</button>';
    $("#gcc-products-container").html(resetHtml).show();
  }

  // Make functions globally available
  window.resetChatbot = resetChatbot;
  window.closeCalendlyAndShowReset = closeCalendlyAndShowReset;
  ...
  function showAnswerOptions(question) {
    ...
    options.forEach((option, index) => {
      ...
      optionsHtml += `
        <button class="gcc-inline-option-btn gcc-option-btn" data-question-id="${question.id}" data-value="${option.value}" data-display="${option.display || option.label || option.text}">
          ${optionText}
        </button>
      `;
    });
